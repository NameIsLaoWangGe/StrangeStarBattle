...
export default class EndlessManage {
    ...
    setSceneElement(status?: string) {
        ...
        arr.forEach((item, index) => {
            if (item.name === "HpLevel") {
                ...
                const labelArray = ["label_score", "label_distance"];
                let i = 0;
    ...
    getDistance() {
        ...
        if (!this.isBossFighting && Date.now() - this.startTime >= this.CALTIMEINTERVAL) {
            ...
            this.startTime = Date.now();
            this.distanceObj.value = "" + this.distanceNum;
    ...
    createSelectSkill() {
        this.isLookedAD = true;
        const arr = EndlessParseSkill.getInstance().getRandomUpgradeSkills();
        //调用打开的界面
    }
    setLookedAdScore() {
