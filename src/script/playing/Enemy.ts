...
export default class Enemy extends Laya.Script {
     ...
     * 爆头
     */
    spike_endless(value: number) {
        const skillInstance = EndlessParseSkill.getInstance();
        if (this.gameMode === "endless" && skillInstance.isUpgraded(14)) {
     ...
     * 大幅增强火力
     */
    enhanceFire(value: number) {
        const skillInstance = EndlessParseSkill.getInstance();
        if (this.gameMode === "endless" && skillInstance.isUpgraded(17)) {
    ...
    initRollBack_endless() {
        ...
        if (this.propertyObj.e_type !== 2 && this.gameMode === "endless") {
            ...
            if (skillInstance.isUpgraded(13) && !this.isRollBack_endless && !this.isImmunityTui_endless) {
                ...
                Laya.Tween.to(this.self, { x: this.self.x, y: this.self.y }, 500, Laya.Ease.circOut, Laya.Handler.create(this, () => {
