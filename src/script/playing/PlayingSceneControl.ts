...
export default class PlayingControl extends Laya.Script {
    ...
    parseTemplate() {
        ...
        const templetArr = ["buff", "diancipao", "xuhongyinqing", "liuxingpao", "mogupao", "feirenfengbao", "daodanpao", "qpbz", "fanghudun"];
        let i: number = 0;
    ...
    */
    ...
    setBgUpdate(): void {
        if (this.isGamePause || PlayingVar.getInstance().gameStatus === "settlement") {
            return;
        }
        this.bg_img1.y += 1;
        this.bg_img2.y += 1;
    ...
    createEnemy() {
        ...
        if (this.isABigWave && nowWaveFinish && !BattleParse.getInstance().judgeNextIsBoss()) {
        ...
        if ((!this.lastTimeCreateOne || judge1 || judge2 || judge3) && this.fighting) {
            if (!this.lastTimeCreateAWave) {
                this.lastTimeCreateAWave = Date.now();
            }
            let enemyId = BattleParse.getInstance().getNextMonsterData();
