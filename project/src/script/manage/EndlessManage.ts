...
export default class EndlessManage {
    ...
    setSceneElement(status?: string) {
        const mainInstance = PlayingControl.instance;
        if (status && status === "exit") {
            mainInstance.secondRoleObj.visible = true;
        } else {
            // PlayingControl.instance.secondRoleObj.visible = false;
            const leftSecondWeapon = mainInstance.leftSecondWeapon;
            const rightSecondWeapon = mainInstance.rightSecondWeapon;
    ...
    leftWeaponmoveFinish(obj: Laya.Image) {
        PlayingControl.instance.secondRoleObj.visible = false;
        obj.x = 25;
        obj.y = 10;
    }
    rightWeaponmoveFinish(obj: Laya.Image) {
        PlayingControl.instance.secondRoleObj.visible = false;
        obj.x = 210;
        obj.y = 10;
    }
    ...
    setUpdateEffect() {
        ...
        if (shengjiObj) {
            ...
            //     return sk;
            // }, this);
            const skObj: Laya.Skeleton = new Laya.Skeleton();
            skObj.load("dragonbones/shengji.sk", Laya.Handler.create(this, (mark) => {
                mark.play(0,false);
            }),0);
            ...
            skObj.on(Laya.Event.STOPPED, this, (temp: Laya.Skeleton) => {
                const markSk: Laya.Skeleton = temp.removeSelf() as Laya.Skeleton;
