...
export default class PlayingControl extends Laya.Script {
    ...
    ItemDetail: LYprefab;
    /** @prop {name:UpdateWeapon,tips:"UpdateWeapon",type:Prefab} */
    UpdateWeapon: LYprefab;
    ...
    onEnable(): void {
        ...
        this.clearSceneChildRes(GameConfig.startScene, true);
        // this.sendHttp();
        this.initSomeVarObj();
        ...
        (this.self["startSence"] as Laya.Animation).play(0, false);
        console.error("进入主场景==============================");
    }
    /**
     * 针对微信小游戏，调整屏幕右上角的显示
    ...
    adapteByWX() {
        ...
        //     return;
        // }
        const diamondStrs = ["img_progressPlane", "progressLeftLabel", "progressRightLabel", "progressBarEnemyNum", "progressLevel"];
        diamondStrs.forEach((item, index) => {
            this.self[item].y += 60;
        });
    }
    onResize(arg: any) {
    ...
    wxUserDateUpdate() {
        if (Laya.Browser.onMiniGame) {
            let args = { type: 2, data: { level: Laya.Browser.window.game.nowLevel, scores: PlayingVar.getInstance().achieve["endlessTopScore"] } };
    ...
    parseTemplate() {
        ...
        const templetArr0 = SkeletonTempletManage.getInstance().TEMPLETARR0/*["buff", "diancipao", "xuanfupao", "xuhongyinqing", "liuxingpao", "mogupao", "feirenfengbao", "daodanpao", "qpbz", "fanghudun", "jiguangpao", "duye"]*/;
        ...
        const bossSkillEffect = SkeletonTempletManage.getInstance().BOSSSKILLEFFECT/*["xuanfeng", "boss2_pao", "baozi", "zd", "boos4texiao", "bingzhui", "boss5texiao"]*/;
        i = 0;
    ...
    loadTempletFinish() {
        if (++this.preResIndex === this.preResNum) {
            PlayingVar.getInstance().preResLoaded = true;
    ...
    loadTempletFinish_enemy() {
        if (++this.preResIndex_e === this.preResNum_e) {
            PlayingVar.getInstance().preResLoaded_e = true;
     ...
     * 预加载部分战斗中里面需要用到的资源
     */
    preLoadResInPlaying(loadNext?: boolean, markLevel?: number) {
        /**
         * 预加载敌人sk模板数据
         */
        const ids = BattleParse.getInstance().getTempletDateInLevel(loadNext, markLevel);
        const templets = SkeletonTempletManage.getInstance().templets;
        let i: string;
    ...
    clearBgRes(bgName: string) {
        const nameNum = Number(bgName.split("_")[1]);
        if (nameNum > 10) {
            let i: number = 10;
            while (i < nameNum) {
                const needClearName = "face/" + "bg_" + i + ".png";
                const textureMark = Laya.loader.getRes(needClearName);
                if(textureMark){
    ...
    */
    ...
    touchStartEndless(e: Laya.Event) {
        ...
        if(Laya.Browser.window.game.nowLevel<10){
            toast.noBindScript("关卡模式达到10关开启！");
            return;
        }
    ...
    waitingLoad() {
        if (this.isWaiting) {
            if (/*(Date.now() - this.waitingStartTime >= 5000) || */(PlayingVar.getInstance().preResLoaded && PlayingVar.getInstance().preResLoaded_e)) {
    ...
    judgeLoadFinish() {
        if (!PlayingVar.getInstance().preResLoaded || !PlayingVar.getInstance().preResLoaded_e) {
    ...
    setBgUpdate(): void {
        ...
        const bg_imgs = [this.bg_img1, this.bg_img2, this.bg_img3];
    ...
    setRoleSuccess() {
        ...
        Laya.Tween.to(this.roleObj, { y: this.roleObj.y - 1600 }, 3000, Laya.Ease.expoIn, Laya.Handler.create(this, () => {
    ...
    judgeBulletType1() {
        ...
        for (i in this.buffData) {
            if (this.buffData[i].type === 1) {
                const repeatJsonNum = this.buffData[i].repeatJsonNum;
    ...
    createBullet() {
        ...
        const buffValue = this.judgeBulletType1();
        if (buffData && buffValue) {
            //弹幕
            const parityBulletNum: number = (buffValue + 4) % 2;
            ...
            for (j; j < buffValue + 4; j++) {
                ...
                const obj = new BulletMain(BulletType.roleMainBullet, position, "普通子弹"/*buffData[i].bullet*/);
            }
        } else {
    ...
    createBulletSecond_muzzle() {
        ...
        let prefabClass: LYprefab = Laya.loader.getRes("prefab/" + prefabName + ".json");
        let i = 0;
        for (i; i < 2; i++) {
    ...
    setBuff(proppertyObj: Buff): void {
        ...
        if (!sameTypeId) {
            ...
            if (this.buffData[buffId].repeatJsonNum) {
                const repeatJsonNum = this.buffData[buffId].repeatJsonNum;
    ...
    setBuffRepeatData(buffData: any) {
        const repeatJsonNum = buffData.repeatJsonNum;
        const buffConfigs = Data.FixedDataTables.getInstance().getData(Data2.DataType.buff);
        switch (buffData.type) {
            ...
            case 2:
                ...
                for (i = 0; i < repeatJsonNum.length; i++) {
                    const markId = repeatJsonNum[i];
                    if (idsIndex[markId]) {
                        idsIndex[markId]++;
                    } else {
                        idsIndex[markId] = 1;
                    }
                    const temp = buffConfigs[markId].value.split("#")[idsIndex[markId] - 1];
            ...
            case 3:
                ...
                for (i = 0; i < repeatJsonNum.length; i++) {
                    ...
                    const temp = buffConfigs[markId].value.split("|")[1].split("#")[idsIndex[markId] - 1];
                    buffData["repeateData"] += Number(temp);
                }
                break;
            default:
    ...
    removeBulletInStage() {
        // Laya.Pool.clearBySign("Bullet_boss");
        (this.self["bulletParent_enemy"] as Laya.Sprite).removeChildren();
    ...
    resumeGame() {
        ...
        this.EnemySpite._children.forEach((item, index, array) => {
            const sk = (item.getChildByName("EnemySK") as Laya.Skeleton);
            if (sk) {
                sk.playbackRate(1);
            } else {
    ...
    createWaveEffect() {
        const warning: LYprefab = Laya.loader.getRes("prefab/Boss_warning.prefab");
    ...
    private signToTimeAndNum = { "monster": { time: null, num: 30 }, "Bullet_boss": { time: null, num: 60 } };
    ...
    createCycleObject(markSign) {
        if (Date.now() - this.signToTimeAndNum[markSign].time >= this.CREATE_CYCLE_INTERVAL) {
            ...
            const resUrl = (markSign === "monster") ? "prefab/EnemySK.json" : "prefab/" + markSign + ".json";
            const markPrefab: LYprefab = Laya.loader.getRes(resUrl);
     ...
     */
    clearSceneChildRes(sceneUrl: string, texture?: boolean) {
        if (sceneUrl) {
            
            const sceneData = Laya.loader.getRes(sceneUrl);
            const loadList: string[] = sceneData.loadList;
            if (loadList) {
                let i: number = 0;
                for (i; i < loadList.length; i++) {
                    if (texture) {
                        Laya.loader.clearRes(loadList[i]);
                    } else {
            ...
            //强制gc
            if(sceneUrl===GameConfig.startScene){
     ...
     * 手动清理》512x512的图片
     */
    clearResUrl(resUrl: string[], texture?: boolean) {
        let i: number = 0;
