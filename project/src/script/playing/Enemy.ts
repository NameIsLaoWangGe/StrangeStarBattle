...
export default class Enemy extends Laya.Script {
    ...
    onEnable(): void {
        ...
        if (this.markSk) {
            ...
            const boss_sk = (this.self.getChildByName(this.self.name) as Laya.Skeleton);
            if (boss_sk) {
    ...
    keepDropHpPlayEffect() {
        ...
        const effectObj = this.self.getChildByName(effectName);
        if (effectObj) {
    ...
    initRollBack_endless() {
        ...
        if (this.propertyObj.e_type !== 2 && this.gameMode === "endless") {
            ...
            if (skillInstance.isUpgraded(13) && !this.isRollBack_endless && !this.isImmunityTui_endless) {
                ...
                Laya.Tween.to(this.self, { x: this.self.x, y: this.self.y - this.rollBackDistance_endless }, 400 / 3, Laya.Ease.circOut, Laya.Handler.create(this, () => {
    ...
    onDisable(): void {
        ...
        if (this.propertyObj && this.propertyObj.e_type && this.propertyObj.e_type === 2) {
            //螃蟹怪 的警告
            const focusRoleObj = mainInstance.roleObj.getChildByName("focusRole");
            if (focusRoleObj) {
