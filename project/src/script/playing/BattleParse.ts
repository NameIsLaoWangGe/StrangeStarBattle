...
export default class BattleParse {
    ...
    endlessSetNextData() {
        ...
        this.initWaveData();
        if (this.nowLevel >= 3) {
            const arr = this.getTempletDateInLevel(null, this.nowLevel - 3);
            this.clearLastLevelRes(arr);
        }
        PlayingSceneControl.instance.preLoadResInPlaying(true);
    }
    clearLastLevelRes(json) {
        let i: string;
        for (i in json) {
            const name = FixedDataTables.getInstance().getDataByKey(Data2.DataType.monster, i, "pic");
            SkeletonTempletManage.getInstance().clearTextureRes(name);
        }
    }
    /**
     * 当前波是否出完所有怪
     ...
     */
    public getTempletDateInLevel(isNext?: boolean, markLevel?: number) {
        let idsJson = {};
        ...
        if (isNext) {
            ...
            if (markLevel) {
                waveMonsterInLevel = FixedDataTables.getInstance().getDataByKey(Data2.DataType.endless, markLevel, "troop").split(";");
            } else {
