...
export default class EquipUpdate extends Laya.Script {
    ...
    onUpdateBtn(b: any, i: number): void {
        ...
        if (this.tab.selectedIndex) {
            ...
            const key = this.game.secondWeapon.selected;
            const args: any = { uuId: PlayingVar.getInstance().uuId, bagSecondaryWeapon: { [key]: changeArg }/*{ [201 || this.game.secondWeapon.selected]: changeArg }*/ };
            console.error("参数打印~！！！~~~~", args);
        ...
        switch (index) {
            case 0:
                if (type === 0) {
                    ...
                    //变成实际的副武器的图标
                    const moneyIcon = FixedDataTables.getInstance().getDataByKey(DataType.secondaryWeapon, this.game.secondWeapon.selected, "pic") + "_shop";
    ...
    public hideAllList(): void {
        ...
        PlayingControl.instance.clearResUrl(["face/装备信息底条.png"], false);
    }
     ...
     * 增加升级武器得特效
     */
    addUpdateEffect(type) {
        //升级武器的特效暂时屏蔽
        return;
        if (type) {
            const posJson = { 0: { x: -39, y: 54 }, 1: { x: 157, y: 54 } };
            for (let i = 0; i < 2; i++) {
                const aniObj: Laya.Skeleton = this.getUpdateWeaponObj();
                aniObj.pos(posJson[i].x, posJson[i].y);
                PlayingControl.instance.secondRoleObj.addChild(aniObj);
            }
        } else {
            const aniObj: Laya.Skeleton = this.getUpdateWeaponObj();
            aniObj.pos(65, 0);
            PlayingControl.instance.roleObj.addChild(aniObj);
        }

    }
    getUpdateWeaponObj() {
        const prefab: LYPrefab = PlayingControl.instance.UpdateWeapon;
        const aniObj: Laya.Skeleton = Laya.Pool.getItemByCreateFun("UpdateWeapon", prefab.create, prefab);
        return aniObj;
    }
}
