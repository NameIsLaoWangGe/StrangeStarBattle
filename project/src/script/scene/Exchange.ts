 ...
 */
    ...
    setExchangeEffect(bagCommon: any) {
        let url: string;
        let itemId: string;
        let endInPos: any;
        if (this.openType) {
        ...
        const lastValue = BagDataControl.getInstance().getBagDataById(itemId).num;
        const nowValue = bagCommon[itemId].num;
        const plusStr = (nowValue - lastValue);
        const startPos = new Laya.Point(Laya.stage.width / 2, Laya.stage.height / 2);
        const endPos = PlayingControl.instance.moneySprite.localToGlobal(endInPos);
        const fly = new GoldFly(startPos, endPos, null, null, null, null, null, plusStr);
    ...
    onDestroy() {
        const urls: string[] = ["commonPic/标题底.png", "exchange/二级底板.png", "exchange/TIPS底板.png", "exchange/信息布局.png", "exchange/信息布局_体力.png", "exchange/底板.png"];
