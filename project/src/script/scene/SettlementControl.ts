...
export default class Settlement extends Laya.Script {
    ...
    initVar() {
        ...
        (self["btn_rewardTwo"] as Laya.Button).on(Laya.Event.CLICK, this, this.lookAd, [1]);
    ...
    lookAd() {
        if (WeiXinAD.getInstance().adStatus) {
            WeiXinAD.getInstance().addVideoAD(Laya.Handler.create(this, this.adFinish));
        } else {
            this.rewardClick(1);
        }
    }
    ...
    showLuckyValue() {
        ...
        let luckyValue = Number(addLuckyNum) + PlayingVar.getInstance().luckyValue;
        if (luckyValue > 100) {
            luckyValue = 100;
        }
        const luckyBate = /*luckyValue*/100 / 100;
    ...
    onDestroy() {
        ...
        const url = this.self.url;
        PlayingControl.instance.clearSceneChildRes(url, false);
