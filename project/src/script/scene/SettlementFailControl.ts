...
export default class SettlementFailControl extends Laya.Script {
    ...
    onEnable(): void {
        ...
        (this.self["btn_rewardTwo"] as Laya.Button).on(Laya.Event.CLICK, this, this.goLookAD, [1]);
    ...
    openTipsAndBack(data: any) {
        if (data.ret) {
            const errorObj = new HttpModel3.HttpError(data.ret, this.owner);
            return;
        }
        var tip: string;
        if (this.receiveType) {
    ...
    clickRevival(e: Laya.Event): void {
        ...
        if (WeiXinAD.getInstance().adStatus) {
            ...
            Laya.timer.pause();
            WeiXinAD.getInstance().addVideoAD(Laya.Handler.create(this, this.revivalFinish));
        } else {
            Laya.timer.once(500, this, () => {
     ...
     * 复活观看广告的回调
     */
    private isRevival: boolean;
    revivalFinish(suc?: number) {
        Laya.timer.resume();
        Laya.timer.once(500, this, () => {
    ...
    onDestroy() {
        ...
        PlayingControl.instance.clearSceneChildRes(this.self.url, false);
        // SkeletonTempletManage.getInstance().releaseAllTemplet();
        if (this.isRevival) {

            PlayingControl.instance.resumeGame();
            role.instance.setRoleRevival();

            this.isRevival = null;
        } else {
            SkeletonTempletManage.getInstance().clearTextureRes();
            Laya.Scene.open("test/FacePlaying.scene", true);
        }
